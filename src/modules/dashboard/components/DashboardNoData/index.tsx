...
export default function DashboardNoData() {
  ...
  return (
    <ContentWrapper className="DashboardNoData" withFullHeight={true} withBackButton={true}>
      <Caption title={intl.formatMessage(messages.title)} description={intl.formatMessage(messages.description)} />
      ...
      <style jsx={true} global={true}>{`
        .DashboardNoData {
          &__markets {
            &:after {
              background: ${isCurrentThemeDark ? currentTheme.lightBlue.hex : sm ? currentTheme.disabledGray.hex : currentTheme.mainBg.hex};
            }
          }
          &__bottom--text {
